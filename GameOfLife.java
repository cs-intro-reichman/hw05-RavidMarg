...
public class GameOfLife {
	...
	public static int[][] read(String fileName) {
		...
		for (int i = 1; i <=rows; i++){
			String line_in_file = in.readLine();
			if (line_in_file != null) {
				for (int j = 1; j <=line_in_file.length(); j++) {
					if (line_in_file.charAt(j-1) == 'x'){
						board[i][j] = 1;
					}
				}
			}
	...
	public static int[][] evolve(int[][] board) {
		int[][] evolved_board = new int[board.length][board[1].length];
		for (int i = 1; i < board.length - 1; i++) {
			for (int j = 1; j < board[i].length - 1; j++) {
				evolved_board[i][j] = cellValue(board, i, j);
			}
		}
	...
	public static int cellValue(int[][] board, int i, int j) {
		...
		int num_of_neighbors = count(board, i ,j);
		if ((cell_value == 1 && (num_of_neighbors == 2 || num_of_neighbors == 3 )) || (cell_value == 0 && num_of_neighbors == 3)){
			cell_value = 1;
		} else if (cell_value == 1 && (num_of_neighbors < 2 || num_of_neighbors > 3)) {
			cell_value = 0;
		}
		return cell_value;
	}
